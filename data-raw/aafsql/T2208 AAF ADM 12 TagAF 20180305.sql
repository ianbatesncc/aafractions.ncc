...
AS (
	SELECT
		...
		--, IP.Age_On_Admission						AS Age_SYOA
		, CASE
...
--SELECT top 1000 * FROM cte_filter_ip ORDER BY Gender ASC, Age_SYOA__adj ASC;
,
...
SELECT
	...
	FROM 
		cte_filter_ip								AS previous_cte
			...
			LEFT JOIN [ucs-bisqldb].[Shared_Reference].[dbo].[Age_Bands_Public_Health]		AS ABPH
				ON (
					(ABPH.[ESP_Year] = '2013') AND
					(ABPH.Age_Years = previous_cte.Age_SYOA__adj)
				)
				
			LEFT JOIN [ucs-bisqldb].[Shared_Reference].[dbo].[Administrative_Codes]			AS ACGENDER
				ON (
...
--SELECT TOP 100 * FROM cte_tag__age_gender
,
/*
...
--select * from cte_tag_af where af is null ;
/*
...
select top 100 * from tmpIB__AA__PHIT_IP__melt ;
;

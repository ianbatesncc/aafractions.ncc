...
--set @analysismethod = 'alcohol-related (narrow) pos1override'
...
SELECT
	...
	-- clinical
	, PHIT_IP.af						AS [af]
	, PHIT_IP.[uid]						AS [condition uid]
--	, AAC.[condition attribution]		AS [condition cat1a]
--	, AAC.[cause basis]					AS [condition cat1b]
--	, AAC.[condition cat2]
--	, AAC.[condition desc]
...
,
/*
Interim step to calculate 'under 18' total

 - RECORD level
*/
cte_tag_u18(data_source, date_start, date_end, [aa method]
	, GenderC, [AgeBand_ESP]
	, af, [condition uid]
	, LADCD, LSOA11CD
	)
...
,
/*
Interim step to calculate 'all ages' total

 - UNION the 'under 18' total too
 - [af] AGGREGATED over ([condition uid], [GenderC], [AgeBand_ESP], [LADCD], [LSOA11CD]
*/
cte_aggegate_over__ageband(data_source, date_start, date_end, [aa method]
	, GenderC, [AgeBand_ESP]
	, [condition uid]
	, LADCD, LSOA11CD
...
SELECT
	...
	FROM (
		...
		SELECT data_source, date_start, date_end
			, [aa method]
			, GenderC, 'All ages' as [AgeBand_ESP]
			, af, [condition uid]
			, LADCD, LSOA11CD
			FROM cte_filter
...
,
/*
Interim step - add in 'P'(ersons) gender total

- (nRecords, nAttributable) AGGREGATED at ([condition uid], [GenderC], [AgeBand_ESP], [LADCD], [LSOA11CD]
*/
cte_sump(data_source, date_start, date_end
	, [aa method]
	, GenderC, [AgeBand_ESP]
	, [condition uid]
	, LADCD, LSOA11CD
...
,
/*
Tag on condition details

- (nRecords, nAttributable) AGGREGATED at ([condition ...], [GenderC], [AgeBand_ESP], [LADCD], [LSOA11CD])
*/
cte_tag_aac(data_source, date_start, date_end
	, [aa method]
	, [GenderC], [AgeBand_ESP]
	, [condition cat1a], [condition cat1b], [condition cat2], [condition desc]
	, LADCD, LSOA11CD, nRecords, nAttributable)
AS (
	SELECT
		data_source, date_start, date_end, [aa method]
		, GenderC, [AgeBand_ESP]
		, AAC.[condition attribution]		AS [condition cat1a]
		, AAC.[cause basis]					AS [condition cat1b]
		, AAC.[condition cat2]
		...
		GROUP BY
			...
			, [aa method]
			, GenderC, AgeBand_ESP
			, AAC.[condition attribution]
			, AAC.[cause basis]
			, AAC.[condition cat2]
			, AAC.[condition desc]
			, LADCD, LSOA11CD
) -- /cte_tag_aac
SELECT /*TOP 1000*/ * FROM cte_tag_aac ;

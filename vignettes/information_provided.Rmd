...
`lu_conditions`, `lu_versions` and `lu_fractions`.

## Conditions

`lu_conditions` is a data.frame object that lists all the alcohol attributable conditions that are present in the lookup tables.  

```{r}
glimpse(lu_conditions)
```
...
knitr::kable(
    caption = "lu_conditions (pseudo-random rows)"
    , sample_n(lu_conditions, 4)
)
...
`lu_fractions` is a data.frame object that lists all the alcohol attributable fractions for each of the alcohol attributable conditions.

```{r}
glimpse(lu_fractions)
```

```{r}
list("aa_ageband", "sex", "analysis_type", "Version") %>% 
    lapply(function(x, y){unique(select(y, x))}, y = lu_fractions) %>% 
...
`lu_versions` is a data.frame object that summarises which conditions are used in each version of the lookup tables.

```{r echo = FALSE}
#knitr::kable(data.table::dcast(lu_versions, ... ~ Version, value.var = "condition_fuid", fun = function(x){any(as.logical(x))}))

knitr::kable(
    caption = "`lu_conditions` (pseudo-random rows)"
    , sample_n(lu_versions, 6)
)

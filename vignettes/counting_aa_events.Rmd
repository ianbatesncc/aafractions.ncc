...
library("kableExtra")
source(devtools::package_file("./data-raw/create_dummy_hesip.R"))
...
c1 <- aa_conditions %>%
    merge(aa_versions %>% filter(Version == "aaf_2017_phe"), by = "condition_uid") %>%
...
aa_fractions %>%
    ...
    merge(aa_conditions %>% select(condition_uid, desc)) %>% 
...
```{r}
ip <- create__dummy_hesip(n = 1024)
glimpse(ip)
```

Some analysis decisions to make about reducing the problem space but still getting the job done.

The first problem is to consider how to process the diagnosis fields.  This approach splits the concatenated diagnoses field into individual diagnosis codes.  This is an expensive resource operation but only needs to be done once.
...
So every record with an alcohol attributable condition has now been assigned an attributable fraction.  Note that one record can have multiple alcohol attributable conditions.  The next stage concerns how to count all these the attributable fractions sensibly.
